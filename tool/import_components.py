...
def generate(components: list[Component]) -> str:
    ...
    lines.append("  List<String> tags,")
    lines.append("})> _presentationComponentSearchTerms = <({")
    lines.append("  String label,")
    lines.append("  String category,")
    lines.append("  String description,")
    lines.append("  List<String> tags,")
    ...
    lines.append("  for (final definition in presentationComponentDefinitions)")
    lines.append("    (")
    lines.append("      label: definition.label.toLowerCase(),")
    lines.append("      category: definition.category.toLowerCase(),")
    lines.append("      description: definition.description.toLowerCase(),")
    lines.append("      tags: <String>[")
    lines.append("        for (final tag in definition.tags) tag.toLowerCase(),")
    lines.append("      ],")
    lines.append("    ),")
    lines.append("];")
    lines.append("")
    ...
    lines.append("  String rawQuery,")
    lines.append(") {")
    lines.append("  final query = rawQuery.trim().toLowerCase();")
    lines.append(
        "  final definitions = presentationComponentDefinitionsSortedByLabel();"
    )
    lines.append("  if (query.isEmpty) return definitions;")
    lines.append("  return definitions.where((definition) {")
    lines.append(
        "    final terms = _presentationComponentSearchTerms[definition.kind.index];"
    )
    lines.append("    return terms.label.contains(query) ||")
    lines.append("        terms.category.contains(query) ||")
    lines.append("        terms.description.contains(query) ||")
    lines.append("        terms.tags.any((tag) => tag.contains(query));")
    lines.append("  }).toList(growable: false);")
    lines.append("}")
    lines.append("")
    return "\n".join(lines)
